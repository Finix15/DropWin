...
fn get_drag_image(image: Image) -> Option<SHDRAGIMAGE> {
    ...
    hbitmap.map(|hbitmap| unsafe {
        ...
        SHDRAGIMAGE {
            ...
            ptOffset: drag_image_cursor_offset(width, height),
...
fn drag_image_cursor_offset(width: i32, height: i32) -> POINT {
    POINT {
        x: width.max(0) / 2,
        y: height.max(0) / 2,
    }
...
#[cfg(test)]
mod tests {
    use super::drag_image_cursor_offset;

    #[test]
    fn drag_image_is_anchored_at_its_center() {
        let offset = drag_image_cursor_offset(256, 256);
        assert_eq!((offset.x, offset.y), (128, 128));
    }

    #[test]
    fn drag_image_offset_handles_odd_and_invalid_dimensions() {
        let odd = drag_image_cursor_offset(255, 127);
        assert_eq!((odd.x, odd.y), (127, 63));

        let invalid = drag_image_cursor_offset(-1, 0);
        assert_eq!((invalid.x, invalid.y), (0, 0));
    }
}
